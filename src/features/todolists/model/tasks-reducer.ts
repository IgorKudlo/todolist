...
export const tasksReducer = createReducer(initialState, (builder) => {
  builder
    ...
    .addCase(deleteTaskAC, (state, action) => {
      ...
      const index = todolist.findIndex(task => task.id === action.payload.taskId)
      if (index !== -1) todolist.splice(index, 1)
    })
    .addCase(createTaskAC, (state, action) => {
      const todolist = state[action.payload.todolistId]
      todolist.unshift({title: action.payload.title, isDone: false, id: nanoid()})
    })
    .addCase(changeTaskStatusAC, (state, action) => {
      const todolist = state[action.payload.todolistId]
      const task = todolist.find(task => task.id === action.payload.taskId)
      if (task) {
